...
///
/// This program helps authomatic analysis of the output file *.sd generated from the random field blume capel model simulation
/// In particular it measures the sizes of the domains
...
////					// BEGIN Store s=0 maps
					...
					if ( Save_S_cooling_ppm == "yes" )
					{
						...
						for ( int ii=0; ii<L; ii++)
						{
							...
							{											
								for ( int kk=0; kk<X.size(); kk++)  if ( ( ii == X[kk] )  && ( jj == Y[kk] ))  OutMapSppm << "255 255 153" << endl ; // Pale Yellow
								for ( int kk=0; kk<Xp.size(); kk++) if ( ( ii == Xp[kk] ) && ( jj == Yp[kk] )) OutMapSppm << "220 20  60 " << endl ; // Crimson
								for ( int kk=0; kk<Xm.size(); kk++) if ( ( ii == Xm[kk] ) && ( jj == Ym[kk] )) OutMapSppm << "70  130 180" << endl ; // SteelBlue
								kkonter++ ;
							}
						}
